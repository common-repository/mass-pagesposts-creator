...
<div class="mmqw-section-left">
	...
	<form id="createForm" method="post" class="mmqw-main-table res-cl">
		...
		<h2><?php esc_html_e( 'Bulk Add Pages/Posts', 'mass-pages-posts-creator' ); ?></h2>
		<table class="form-table">
		    				...
		    				if ( mppcp_fs()->can_use_premium_code() || mppcp_fs()->is_trial() ) {
		    					if ( ! empty( $get_post_types ) && is_array( $get_post_types ) ) {
		    						foreach ($get_post_types as $post_type_list) {
			    						...
			    						?><option value="<?php echo esc_attr( $post_type_list->name ); ?>"><?php echo esc_html( $post_type_list->label ); ?></option><?php
			    					}
		    					}
		    				} else {
		    					if ( ! empty( $get_post_types ) && is_array( $get_post_types ) ) {
		    						foreach ($get_post_types as $post_type_list) {
			    						...
			    						?><option value="<?php echo esc_attr( $post_type_list->name . '_in_pro' ); ?>"><?php echo esc_html( $post_type_list->label . ' 🔒' ); ?></option><?php
			    					}
		    					}
		    				}
		    			} else {
	    	...
	    	?>
					<tr class="template_name_tr">
						<th class="titledesc"><?php esc_html_e( 'Templates', 'mass-pages-posts-creator' ); ?><span class="mppc-pro-label"></span></th>
						<td>
							<select id="template_name" disabled>
								<option value=""><?php esc_html_e( 'Select Template', 'mass-pages-posts-creator' ); ?></option>
							</select>
						</td>
					</tr>
					...
					<select id="page_status">
						...
						if ( mppcp_fs()->is__premium_only() || mppcp_fs()->is_trial() ) {
						    if ( mppcp_fs()->can_use_premium_code() || mppcp_fs()->is_trial() ) {
								...
								?>
								<option value="publish_in_pro"><?php esc_html_e( 'Publish 🔒', 'mass-pages-posts-creator' ); ?></option>
								<?php 
							}
						} else {
							?>
						...
						?>
						<option value="pending" selected><?php esc_html_e( 'Pending', 'mass-pages-posts-creator' ); ?></option>
    	...
    	?>
				<tr class="comment_status_tr">
					<th class="titledesc"><?php esc_html_e( 'Pages/Posts Comment Status', 'mass-pages-posts-creator' ); ?><span class="mppc-pro-label"></span></th>
					<td>
						<select id="comment_status" disabled>
							<option value=""><?php esc_html_e( 'Select Comment Status', 'mass-pages-posts-creator' ); ?></option>
						</select>
					</td>
				</tr>
    	...
    	?>
				<tr class="authors_tr">
					<th class="titledesc"><?php esc_html_e( 'Author', 'mass-pages-posts-creator' ); ?><span class="mppc-pro-label"></span></th>
					<td>
						<select id="authors" disabled>
							<option value=""><?php esc_html_e( 'Select Author', 'mass-pages-posts-creator' ); ?></option>
						</select>
					</td>
				</tr>
    	...
    	?>
				<tr class="parent_page_id_tr">
					<th class="titledesc"><?php esc_html_e( 'Parent Pages', 'mass-pages-posts-creator' ); ?><span class="mppc-pro-label"></span></th>
					<td>
						<?php 
					    $html = '<select id="page-filter-free" name="pages[]" class="pages_values" disabled>';
